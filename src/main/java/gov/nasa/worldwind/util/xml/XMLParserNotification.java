...
public class XMLParserNotification extends PropertyChangeEvent {
	 ...
	 *            the notification type, such as {@link #EXCEPTION} or {@link #UNRECOGNIZED}.
	 * @param event
	 *            if an event is associated with the notification, that event. May be null.
	 * @param msg
	 *            a message from the notification source suitable for logging.
	 * @param oldValue
	 *            any old value associated with the notification. Not typically used.
	 * @param newValue
	...
	public XMLParserNotification(Object source, String notificationType, XMLEvent event, String msg, Object oldValue, Object newValue) {
		...
		if (newValue instanceof Exception) this.exception = (Exception) newValue;
	}
	 ...
	 *            the source to assign the exception.
	 */
	...
	public String toString() {
		...
		if (this.event != null) {
			msg = Messages.getMessage(this.message, this.event.toString(), this.event.getLineNumber());
		} else {
			msg = Messages.getMessage(this.message, "", "", "");
		}
